...
export default function PasienTab({ showToastMsg }: PasienTabProps) {
  ...
  const handleSelectPatient = async (id: string) => {
    ...
    try {
      ...
      showToastMsg('⚠️ Gagal memuat detail pasien.')
    } finally {
                ...
                {patientItems.map((p, idx) => {
                  ...
                  return (
                    <tr
                      ...
                      style={{ borderBottom: '1px solid #F4F5F7', background: idx % 2 === 0 ? '#fff' : '#FAFBFC', opacity: isActive ? 1 : 0.55, cursor: 'pointer', transition: 'background 0.1s' }}
