...
```ts
export function skorToStatus(score: number): RiskStatus {
  if (score <= 35) return 'aman';
  if (score <= 65) return 'waswas';
  return 'bahaya';
}
...
| Rata Skor Risiko | `avg_risk_score` | amber |

**Score Distribution:** 3 angka aman/waswas/bahaya sebagai badge pill + angka.
...
| Eskalasi Aktif | `active_escalations` | danger |
| Rata Skor | `avg_risk_score` | amber |
...
Filter: `risk_status` (semua / waswas / bahaya).
